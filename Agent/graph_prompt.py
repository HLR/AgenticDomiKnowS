...
def get_graph_prompt():
    ...
    You are an expert DomiKnowS knowledge-graph coder **(you only output the graph code without any explanation)**.
...
def get_graph_reviewer_prompt():
    graph_reviwer_instructions = f"""
        ...
        Your goal is to review a graph and provide feedback on its quality in this regards:
        1. Can the constraints be expressed in a simpler way or rather are there constraints that are redundant?
        2. Are there any concepts that are not useful (dont have labels in the dataset, are not part of the graph structure and are not used in constraints) and can they be removed?
        3. Are the relations used correctly? contains has to be 1 to many and has_a many to many.
        4. Are there any of the concepts that are properties and should be removed?
        
        If there is no issue simply say "approve"
    ...
    graph_examples = ["""Problem definition: We have a collections of images with given pixels and dimentions each with some regions. each region has a bounding box, some textual clues and some textual inferences. we want to find the correct inference for each region. If there are at least 3 useful clues related to an inference, that inference must be correct.
      with Graph('sherlockgraph') as graph:
            ...
            with Graph('Conll') as graph:
                ...
                ifL(orgbase_on('x'), andL(organization(path=('x', rel_pair_phrase1)), location(path=('x', rel_pair_phrase2))))
                ifL(kill('x'), andL(people(path=('x', rel_pair_phrase1)), people(path=('x', rel_pair_phrase2))))
            ""","""The word concept is redundant as it's neither a label nor used in any other part of the graph. You can remove it."""
        """ Problem Definition: A simple graph for a binary classification of x where 3 different model make a prediction for it. Not all the predictions can be True at the same time.
               with Graph('X_graph') as graph:
                   ...
                   """,
              """Instead of the 3 constraints one simple constraint notL(andL(andL(x1,x2),x3)) would suffice and is better for explainability."""
              ]
